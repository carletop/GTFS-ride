...
| boardings | **Conditionally Required** | The **boardings** field contains the number of boardings at (or nearest to, in the case of boardings between stops ) the associated **stop_id** as collected by either automated or manual methods. Ridership counts must be recoreded in this field and/or **alightings** if **record_use** is 0. Non-negative integer. |
| alightings | **Conditionally Required** | The **alightings** field contains the number of alightings at (or nearest to, in the case of alightings between stops) the associated **stop_id** as collected by either automated or manual methods. Ridership counts must be recoreded in this field and/or **boardings** if **record_use** is 0. Non-negative integer. |
...
|  Field Name | Required | Details |
|  ------ | ------ | ------ |
| agency_id | **Conditionally Required** | The **agency_id** field contains the ID of the agency associated with the capacity data. This value is referenced from the [agency.txt](https://github.com/google/transit/blob/master/gtfs/spec/en/reference.md#agencytxt) file. This field is required when the dataset contains data for multiple transit agencies, otherwise it is optional.  |
...
| agency_id | **Conditionally Required** | The **agency_id** field contains the ID of the agency associated with the unique rider. This value is referenced from the [agency.txt](https://github.com/google/transit/blob/master/gtfs/spec/en/reference.md#agencytxt) file. This field is required when the dataset contains data for multiple transit agencies, otherwise it is optional.  |
...
|  Field Name | Required | Details |
|  ------ | ------ | ------ |
| total_boardings | **Conditionally Required** | The **total_boardings** field contains the total count (not a daily average) of all boardings for the identified service or stops for the period indicated. A record without a **stop_id** should have both **total_boardings** and **total_alightings**, and they should be equal; a record with a **stop_id** must have at least one of the two. Non-negative integer. |
| total_alightings | **Conditionally Required** | The **total_alightings** field contains total count (not a daily average) of all alightings for the identified service or stops for the period indicated. A record without a **stop_id** should have both **total_boardings** and **total_alightings**, and they should be equal; a record with a **stop_id** must have at least one of the two. Non-negative integer. |
